...
static void prvTask_Notified(void *pvParameters)
{
    ...
    for (;;)
    {
                ...
                portMAX_DELAY))
        {
            ...
            {
                xTaskCreate(prvTask_logger, "tasklogger", configMINIMAL_STACK_SIZE, NULL, TASK3_PRIORITY, &prvTask_logger_hldr);
            }
            if (notificationValue == 2)
            {
                xTaskCreate(prvTask_coder, "taskcoder", configMINIMAL_STACK_SIZE, 8, TASK3_PRIORITY, &prvTask_coder_hldr);
                xTaskCreate(prvTask_led, "taskLED", configMINIMAL_STACK_SIZE, &green, TASK4_PRIORITY,  &redTask_hdlr);
            }
            else
            {
                printf("               \n");
...
static void prvTask_getChar(void *pvParameters)
{
    ...
    for (;;)
    {
        int stop = 0;
        key = getchar();
        //verifica caracteres conforme tabela ASCII
        //10 = ENTER (salto de linha)
        //27 = ESC (escape)
        //32 = espaço (space)
        //de 97 a 122 = alfabeto a~z
        //de 48 a 57  = numerais 0~9
        if((key==10)||(key==27)||(key==32)||((key>=97)&&(key<=122))||((key>=48)&&(key<=57))) 
        {
            switch (key)
            {
                case 27:
...
static void prvTask_coder(void *pvParameters)
{
    ...
    {      
        ...
        {
            printf("                                                                                           \n");
            gotoxy(cursor_x=cursor_x+1, CURSOR_Y);
            printf("%c", key);
            fflush(stdout);
        }
        else
...
void app_run(void)
{
    ...
    DISABLE_CURSOR();
    printf(
        "╔═════════════════╗ Digitar o texto (alfabeto ou numerais), teclar ENTER para\n"
        "║                 ║ enviar texto a ser codificado ou ESC para encerrar.\n"
        "╚═════════════════╝\n");

    xTaskCreate(prvTask_Notified, "taskNotified", configMINIMAL_STACK_SIZE, NULL, TASK1_PRIORITY, &notified_hdlr);
    xTaskCreate(prvTask_coder2, "taskCoder", configMINIMAL_STACK_SIZE, 5, TASK3_PRIORITY, &loggerTexto_Task_hdlr);
    xTaskCreate(prvTask_getChar, "taskGetchar", configMINIMAL_STACK_SIZE, NULL, TASK2_PRIORITY, NULL);
